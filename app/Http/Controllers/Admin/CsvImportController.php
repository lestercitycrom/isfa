		...
		DB::transaction(function () use ($csv, &$header, $companyId, &$added, &$skipped): void {
			foreach ($csv as $row) {
				...
				if ($product === null) {
					...
					// Update existing product (merge category and description if provided) Ч считаем как пропуск дубл€
					$updateData = [];
					if ($categoryId !== null) {
				...
				if ($supplierInput !== null && $supplierInput !== '') {
					...
					foreach ($supplierItems as $supplierItem) {
						...
						// Try to find by ID if numeric (в рамках текущего пользовател€)
						if (is_numeric($supplierItem)) {
							$supplier = Supplier::query()
								...
								->first();
							if ($supplier === null) {
								// ѕоставщика с таким ID нет у пользовател€ Ч берЄм им€ из глобальной записи и создаЄм своего
								$existing = Supplier::query()->whereKey((int) $supplierItem)->first();
								$supplierName = $existing?->name;
							}
						}

						// If not found by ID, try to find by name (в рамках текущего пользовател€)
						if ($supplier === null) {
							$supplier = Supplier::query()
								->when($companyId !== null, fn ($q) => $q->where('company_id', $companyId))
						...
						// ≈сли у пользовател€ такого поставщика нет Ч создаЄм ему поставщика с этим именем
